...
class Mosaic(BaseMixTransform):
    ...
    def _update_labels(labels, padw, padh):
        ...
        # 新增：保存难度信息
        if hasattr(labels["instances"], 'difficulty') and labels["instances"].difficulty is not None:
            labels["difficulty"] = labels["instances"].difficulty
        return labels
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        Returns:
            (Dict): A dictionary containing concatenated and processed labels for the mosaic image.
        """
        if len(mosaic_labels) == 0:
            return {}
        cls = []
        instances = []
        difficulties = []  # 新增：收集难度信息
        ...
        for labels in mosaic_labels:
            ...
            # 新增：收集难度信息
            if "difficulty" in labels and labels["difficulty"] is not None:
                difficulties.append(labels["difficulty"])
        ...
        # 新增：拼接难度信息
        if difficulties:
            final_labels["difficulty"] = np.concatenate(difficulties, 0)
        ...
        if "difficulty" in final_labels:
            final_labels["difficulty"] = final_labels["difficulty"][good]
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        # 新增：合并难度信息
        if "difficulty" in labels and "difficulty" in labels2:
            labels["difficulty"] = np.concatenate([labels["difficulty"], labels2["difficulty"]], 0)
...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        if difficulty is not None:
            if len(difficulty) == len(i):
                labels["difficulty"] = difficulty[i]
            else:
                # 如果长度不匹配，可能需要处理
                labels["difficulty"] = difficulty[i] if len(difficulty) > 0 else None
...
class CopyPaste(BaseMixTransform):
    ...
    def _transform(self, labels1, labels2={}):
        ...
        for j in indexes[: round(self.p * n)]:
            ...
            # 新增：合并 difficulty ???
            if "difficulty" in labels1 and "difficulty" in labels2:
                labels1["difficulty"] = np.concatenate(
                    (labels1["difficulty"], labels2["difficulty"][[j]]),
                    axis=0
                )
...
class Format:
    ...
    def __call__(self, labels):  # 新增
        ...
        Args:
            labels (Dict): A dictionary containing image and annotation data.

        Returns:
            (Dict): A dictionary with formatted data.
        """
        img = labels.pop("img")
        h, w = img.shape[:2]
        cls = labels.pop("cls")
        ...
        # 新增：获取难度信息
        difficulty = labels.pop("difficulty", None)
        ...
        if difficulty is not None:
            if isinstance(difficulty, np.ndarray):
                labels["difficulty"] = torch.from_numpy(difficulty) if nl else torch.zeros(nl)
            elif isinstance(difficulty, torch.Tensor):
                labels["difficulty"] = difficulty if nl else torch.zeros(nl)
